/*
 * @Author: ZhongJunWei
 * @Date: 2022/07/04 18:00
 * @LastEditTime: 2022/07/05 10:10
 * @LastEditors: ZhongJunWei
 * @FilePath: \vue3-vite2-h5-template-ynzy\src\directive\buttonAuth.ts
	...
	app.directive('auth', {
		mounted(el: any, binding: any, vnode: any) {
			...
			const clickFn = () => {
				...
				const justOneAuth = argAuth.some((item: any) =>
					buttonAuth.some((atem) => item === atem.toLocaleLowerCase())
				)
				if (justOneAuth) {
					if (binding.value) {
						const params = binding.value.params
